...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-signal-sec-policy-detail.ps1

php tests/test_domain_review.php

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-signal-sec-policy-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-signal-sec-policy-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-signal_sec_policy_vault-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
